...
const entities: EntityTestDescription[] = [
    ...
    { entity: InvoiceLine, tableName: "invoice_line", nameAlias: "invoice_line",
        rawMapper: (x) => ({invoiceLineId: x.invoice_line_invoice_line_id, quantity: x.invoice_line_quantity, unitPrice: x.invoice_line_unit_price}),
    ...
    },
    {
        ...
        applyOption: (entity, qb, oracleFix) => {
            if (entity.name === Album.name) {
                ...
                return qb.orderBy(fixOracle("name", oracleFix), "ASC").addOrderBy(fixOracle("track_id", oracleFix), "ASC")
            } else if (entity.name === PlaylistTrack.name) {
        ...
        optionForRepo: (entity) => {
            if (entity.name === Album.name) {
                ...
                return {name: "ASC", trackId: "ASC"} as FindOptionsOrder<Track>
            } else if (entity.name === PlaylistTrack.name) {
        ...
        orderDataset: (entity, dbDialect) => {
            if (entity.name === Album.name) {
                ...
                return (dataset) => dataset.slice().sort((a: Track, b: Track) => datasetOrderDependingOnDialect(dbDialect, a.name, b.name) | (a.trackId - b.trackId))
            } else if (entity.name === PlaylistTrack.name) {
